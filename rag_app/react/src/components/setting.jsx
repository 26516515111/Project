...
export default function Setting({ onBack, chats = [], onClearChats, onDeleteChat }) {
    const [activeTab, setActiveTab] = useState("personal");

    // 用户角色：后端数据库传入，1 = 管理者(admin), 0 = 普通用户(user)
    const [userRole, setUserRole] = useState("admin"); // 实际项目中从后端获取
    ...
    useEffect(() => {
        ...
        // 实时保存到本地缓存，保证退出再进或刷新页面都不会重置
        localStorage.setItem("deepblue_settings", JSON.stringify(settings));
    }, [settings]);
    ...
    const handleKBFileUpload = async (e) => {
        ...
        for (let i = 0; i < uploadedFiles.length; i++) {
            ...
            formData.append("user_id", "admin"); // 与后端的 user_id 对应
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <div className="rounded-xl flex items-center px-3 py-2.5 select-none"
                        ...
                        <span className="text-sm flex-1" style={{ color: "#d1d5db" }}>{userRole === "admin" ? "管理者" : "普通用户"}</span>
                        <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.05)", color: "#6b7280" }}>由系统分配</span>
                    </div>
                    <p className="text-[10px] ml-1 mt-1" style={{ color: "#6b7280" }}>权限级别由后端数据库统一管理，无法自行修改（后端标识：{userRole === "admin" ? "1" : "0"}）</p>
                </div>
    ...
    const renderSystemPrefs = () => (
        <div className="space-y-8 animate-fade-in">
            {/* 主题选择 */}
            <section>
                <h3 className="text-sm font-medium mb-4 flex items-center gap-2" style={{ color: "#e5e7eb" }}>
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[
                        ...
                        <button
                            ...
                            style={{
                                background: settings.theme === t.id ? "rgba(13,148,136,0.1)" : "rgba(255,255,255,0.02)",
                                borderColor: settings.theme === t.id ? "rgba(45,212,191,0.5)" : "rgba(255,255,255,0.05)",
                                boxShadow: settings.theme === t.id ? "0 0 15px rgba(45,212,191,0.1)" : "none"
                            }}
                        >
                            <div className="w-10 h-10 rounded-full flex items-center justify-center"
                                style={{ background: settings.theme === t.id ? "#0d9488" : "rgba(255,255,255,0.05)", color: settings.theme === t.id ? "white" : "#9ca3af" }}>
                                <i className={`ph ${t.icon} text-xl`}></i>
                            </div>
                            <span className={`text-sm ${settings.theme === t.id ? "font-medium" : ""}`} style={{ color: settings.theme === t.id ? "#2dd4bf" : "#9ca3af" }}>
                                {t.label}
                            </span>
                        </button>
                    ))}
            ...
            {/* 字体大小 */}
            <section className="p-6 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-medium flex items-center gap-2" style={{ color: "#e5e7eb" }}>
                        <i className="ph ph-text-aa text-teal-400"></i> 界面字号
                    </h3>
                    <span className="text-xs px-2 py-1 rounded" style={{ background: "rgba(13,148,136,0.1)", color: "#2dd4bf" }}>{settings.fontSize}px</span>
                </div>
                <div className="flex items-center gap-4 mb-4">
                    ...
                    />
                    <span className="text-lg" style={{ color: "#6b7280" }}>A</span>
                </div>
                <div className="p-4 rounded-lg border flex items-start gap-3" style={{ background: "rgba(0,0,0,0.2)", borderColor: "rgba(255,255,255,0.05)" }}>
                    <i className="ph ph-info mt-1" style={{ color: "#9ca3af" }}></i>
                    <p style={{ fontSize: `${settings.fontSize}px`, color: "#d1d5db", lineHeight: 1.6 }}>
            ...
            <section className="space-y-2">
                <h3 className="text-sm font-medium mb-4 mt-2 px-1" style={{ color: "#e5e7eb" }}>消息与记录</h3>

                <div className="flex items-center justify-between p-4 rounded-xl transition-colors hover:bg-white/[0.04]"
                    style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div>
                        <p className="text-sm" style={{ color: "#e5e7eb" }}>消息提示</p>
                        <p className="text-xs mt-0.5" style={{ color: "#6b7280" }}>收到 AI 回复时播放提示音并显示通知</p>
                    </div>
                    <Switch checked={settings.notify} onChange={e => updateSetting("notify", e.target.checked)} />
                </div>

                <div className="flex items-center justify-between p-4 rounded-xl transition-colors hover:bg-white/[0.04]"
                    style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div>
                        <p className="text-sm" style={{ color: "#e5e7eb" }}>自动保存历史记录</p>
                        <p className="text-xs mt-0.5" style={{ color: "#6b7280" }}>退出系统时保留所有未关闭的会话</p>
                    </div>
                    <Switch checked={settings.autoSave} onChange={e => updateSetting("autoSave", e.target.checked)} />
                </div>
            </section>
        </div>
    );
    ...
    const renderAISettings = () => (
        <div className="space-y-6 animate-fade-in">
            {/* 知识图谱默认开关 */}
            <div className="p-6 rounded-2xl" style={{ background: "rgba(13,148,136,0.05)", border: "1px solid rgba(45,212,191,0.2)" }}>
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                        <i className="ph ph-graph text-teal-400 text-xl"></i>
                        <h3 className="text-sm font-medium text-white">默认开启知识图谱分析</h3>
                    </div>
                    <Switch checked={settings.graphOn} onChange={e => updateSetting("graphOn", e.target.checked)} />
                </div>
                <p className="text-xs pl-7" style={{ color: "#9ca3af" }}>新建会话时，默认勾选右侧边栏的"知识图谱分析"功能，结合历史记录深度诊断。</p>
            </div>
            ...
            <div className="p-6 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-medium text-white">默认离线模式状态</h3>
                    </div>
                    <Switch checked={settings.offlineOn} onChange={e => updateSetting("offlineOn", e.target.checked)} />
                </div>
                <p className="text-xs pl-7 mb-4" style={{ color: "#9ca3af" }}>在无网络覆盖海域，默认启动本地轻量模型和离线知识库。</p>

                <div className="mt-4 pt-4 pl-7 border-t" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                    <label className="block text-xs font-medium mb-2" style={{ color: "#6b7280" }}>本地知识库路径配置</label>
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            style={{
                                background: settings.offlineOn ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.02)",
                                borderColor: "rgba(255,255,255,0.1)",
                                color: settings.offlineOn ? "#d1d5db" : "#6b7280",
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-xs font-medium ml-1" style={{ color: "#9ca3af" }}>默认问答模型</label>
                    <GlassSelect
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center ml-1">
                        <label className="text-xs font-medium" style={{ color: "#9ca3af" }}>流式输出速度</label>
    ...
    const renderPrivacySecurity = () => (
        <div className="space-y-6 animate-fade-in">
            ...
            {/* 专家知识库上传 - 仅管理者可见 */}
            <div className="relative p-6 rounded-2xl overflow-hidden group mt-8" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <div className="flex justify-between items-center mb-5">
                    <div>
                        <h3 className="text-sm font-medium text-white flex items-center gap-2">
                            <i className="ph ph-folder-open text-teal-400 text-lg"></i> 专家知识库上传
                        </h3>
                        <p className="text-xs mt-1" style={{ color: "#6b7280" }}>上传 PDF/Word 等资料以丰富图谱库</p>
                    </div>

                    {/* 身份显示（只读标识） */}
                    <div className="flex items-center gap-2 text-xs">
                        <span className="hidden sm:inline" style={{ color: "#6b7280" }}>当前身份:</span>
                        <div
                            className="px-2.5 py-1 rounded-md border flex items-center gap-1.5"
                            style={{
                                background: userRole === "admin" ? "rgba(13,148,136,0.1)" : "rgba(255,255,255,0.05)",
                                borderColor: userRole === "admin" ? "rgba(45,212,191,0.3)" : "rgba(255,255,255,0.1)",
                                color: userRole === "admin" ? "#2dd4bf" : "#9ca3af"
                            }}
                        >
                            {userRole === "admin" ? <i className="ph ph-crown"></i> : <i className="ph ph-user"></i>}
                            {userRole === "admin" ? "管理者" : "普通用户"}
                        </div>
                    </div>
                </div>
                ...
                <input
                    ...
                    className="hidden"
                    ref={kbInputRef}
                    disabled={userRole !== "admin" || kbUploading}
                    onChange={handleKBFileUpload}
                />

                {/* 上传区域 */}
                <div
                    onClick={() => userRole === "admin" && !kbUploading && kbInputRef.current?.click()}
                    ...
                    style={{
                        borderColor: userRole === "admin" ? "rgba(45,212,191,0.3)" : "rgba(255,255,255,0.05)",
                        background: userRole === "admin" ? "rgba(13,148,136,0.05)" : "rgba(0,0,0,0.2)",
                        cursor: userRole === "admin" && !kbUploading ? "pointer" : "default",
                        opacity: kbUploading ? 0.85 : 1,
                    }}
                >
                    <div className="w-12 h-12 rounded-full flex items-center justify-center mb-3"
                        style={{
                            background: userRole === "admin" ? "rgba(13,148,136,0.2)" : "rgba(255,255,255,0.05)",
                            color: userRole === "admin" ? "#2dd4bf" : "#4b5563"
                        }}>
                ...
                {userRole === "admin" && (kbUploading || kbUploadHint) && (
                ...
                {userRole === "admin" && files.length > 0 && (
                    <div className="space-y-2 mt-2">
                        <p className="text-xs font-medium mb-2 px-1" style={{ color: "#6b7280" }}>最近上传</p>
                        {files.map(file => (
                            <div key={file.id} className="flex items-center justify-between p-3 rounded-lg transition-colors group/file hover:bg-white/5"
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs font-medium" style={{ color: "#e5e7eb" }}>{file.name}</p>
                                        <p className="text-[10px] mt-0.5" style={{ color: "#6b7280" }}>{file.size} • {file.date}</p>
                                        {file.detail && (
                                            <p className="text-[10px] mt-0.5" style={{ color: file.status === "error" ? "#fda4af" : file.status === "processing" ? "#7dd3fc" : "#6ee7b7" }}>
                                                {file.detail}
                                            </p>
                                        )}
                                    </div>
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] px-1.5 py-0.5 rounded border"
                                        style={{
                                            color: file.status === "error" ? "#fda4af" : file.status === "processing" ? "#7dd3fc" : "#6ee7b7",
            ...
            {/* 管理指定的历史记录 */}
            <div className="relative p-6 rounded-2xl overflow-hidden group mt-6" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <h3 className="text-sm font-medium flex items-center gap-2 mb-4 text-white">
                    <i className="ph ph-list-dashes text-teal-400 text-lg"></i> 管理特定会话记录
                </h3>
                {chats.length === 0 ? (
                    <p className="text-xs" style={{ color: "#6b7280" }}>暂无任何问答历史记录。</p>
                ) : (
                        ...
                        {chats.map(chat => (
                            <div key={chat.id} className="flex items-center justify-between p-3 rounded-lg transition-colors group/chat hover:bg-white/5"
                                ...
                                <div className="flex items-center gap-3 overflow-hidden">
                                    ...
                                    <p className="text-xs font-medium truncate" style={{ color: "#e5e7eb" }}>{chat.title}</p>
                                </div>
                                <button
    ...
    const renderAboutHelp = () => (
        <div className="space-y-6 animate-fade-in flex flex-col items-center max-w-lg mx-auto mt-4">
            ...
            <div className="w-full flex flex-col items-center p-8 rounded-3xl relative overflow-hidden mb-2"
                ...
                <h2 className="text-xl font-bold text-white tracking-wide mb-1 relative z-10">智能船舶问答系统</h2>
                <p className="text-xs font-mono px-2 py-0.5 rounded border mb-4 relative z-10"
                    style={{ background: "rgba(13,148,136,0.1)", color: "#2dd4bf", borderColor: "rgba(45,212,191,0.2)" }}>
                    Version 2.4.1 Build 8829
                </p>
                <p className="text-xs" style={{ color: "#6b7280" }}>上次更新: 2024-01-15</p>
            ...
            {/* 意见反馈 */}
            <div className="w-full p-6 rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <h3 className="text-sm font-medium mb-4 flex items-center gap-2" style={{ color: "#e5e7eb" }}>
            ...
            <div className="w-full flex items-center justify-between px-4 py-3 rounded-xl"
                style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <span className="text-xs" style={{ color: "#6b7280" }}>后端连接状态</span>
                <div className="flex items-center gap-2 px-2.5 py-1 rounded-full border"
                    ...
                    <span className="text-[11px] font-medium" style={{ color: "#2dd4bf" }}>运行中</span>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="flex h-screen w-full" style={{ background: "#030d17", color: "#d1d5db", margin: 0, overflow: "hidden" }}>
            {/* ========== 左侧边栏 ========== */}
            <aside className="w-64 z-20 flex flex-col h-full shrink-0"
                ...
                <div className="flex-1 overflow-y-auto px-3 flex flex-col gap-1 mt-2"
                    ...
                    <p className="px-2 text-xs font-medium mb-2 mt-2" style={{ color: "#6b7280" }}>设置菜单</p>
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                ...
                <header className="h-16 flex items-center justify-between px-8 shrink-0 relative z-20"
                    ...
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                            ...
                            <i className={`ph ${tabs.find(t => t.id === activeTab)?.icon}`} style={{ color: "#9ca3af" }}></i>
                        </div>
                        <h1 className="text-base font-medium" style={{ color: "#f3f4f6" }}>{currentTabLabel}</h1>
                    </div>
