...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--to", dest="end", type=int, default=6, help="运行到第几步结束")
    ...
    parser.add_argument("--only-doc", dest="only_doc_id", type=str, default=None, help="step3 仅处理指定 doc_id")
    parser.add_argument("--no-context", action="store_true", help="step3 关闭上下文注入")
    parser.add_argument("--no-checkpoint", action="store_true", help="step3 不使用 checkpoint")
    parser.add_argument("--no-neo4j-import", action="store_true", help="step5 仅生成 CSV，不导入 Neo4j")
    parser.add_argument("--no-neo4j-dump", action="store_true", help="step5 导入后不导出 dump")
    parser.add_argument("--visualize-top", type=int, default=300, help="step6 最多展示节点数")
    parser.add_argument("--visualize-label", type=str, default=None, help="step6 仅展示指定标签")
...
def main() -> int:
    ...
    if migration["moved"] or migration["removed_duplicates"]:
        print(f"已迁移旧文件: {len(migration['moved'])}，清理重复文件: {len(migration['removed_duplicates'])}")
